...
-(void)createMarker:(CDVInvokedUrlCommand *)command
{
    ...
    // if clustering is active
    if (self.mapCtrl.clusterManager != nil) {
        
        //ID of the cluster item (marker)
        if ([json valueForKey:@"id"]) {
            name = [json valueForKey:@"id"];
        }
        // icon for the marker inside the clustering
        ...
        item = [[POIItem alloc] initWithPosition:CLLocationCoordinate2DMake(latitude, longitude)
                                            name: name imageUrl: iconPath];
        // add the item to the cluster manager and cluster the map (group markers)
        [self.mapCtrl.clusterManager addItem:item];
        [self.mapCtrl.clusterManager cluster];
        
        NSString *id = [NSString stringWithFormat:@"marker_%@", name];
        NSMutableDictionary *result = [[NSMutableDictionary alloc] init];
        [result setObject:id forKey:@"id"];
        [result setObject:[NSString stringWithFormat:@"%@", name] forKey:@"hashCode"];
...
-(void)setIcon:(CDVInvokedUrlCommand *)command
{
    ...
    if (self.mapCtrl.clusterManager != nil) {
        NSString* poiItemId = [markerKey stringByReplacingOccurrencesOfString:@"marker_" withString:@""];
        
        // `url` property
        NSString* iconPath = [iconProperty valueForKey:@"url"];
        [self.mapCtrl.clusterManager changeImageUrlForPOIItemAtIndex:poiItemId newImageUrl: iconPath];
