...
- (UIImage *)iconForSize:(NSUInteger)size {
    ...
    if (size < _buckets[0].unsignedLongValue) {
        ...
        // If it's bigger than the max calue, display the correct value
        NSUInteger maxIndex = [_buckets count] -1;
        if (size > _buckets[maxIndex].unsignedLongValue) {
            text = [NSString stringWithFormat:@"%ld", size];
        } else {
