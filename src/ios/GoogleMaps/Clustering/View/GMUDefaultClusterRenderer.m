...
- (GMSMarker *)markerWithPosition:(CLLocationCoordinate2D)position
    ...
    if (clusterIcon != nil) {
        ...
        if ([userData isKindOfClass:POIItem.class]) {
            POIItem *poiItem = (POIItem*) userData;
            NSString* imagePath = poiItem.imageUrl;
            marker.icon = [self getImageFromPath:imagePath];
        }
    }
...
// TODO fix this, need to handle something more than file://  url for path of image
- (UIImage*) getImageFromPath:(NSString*) iconPath {
...
- (void) changeImageUrlForPOIItemAtIndex: (NSString*) poiItemId newImageUrl:(NSString*) imageUrl{
    POIItem* item;
    for (GMUStaticCluster* currentItem  in _clusters){
        POIItem* currentPOIItem = currentItem.items[0];
        if ([currentPOIItem.name isEqualToString: poiItemId]) {
            item = currentPOIItem;
            [self renderCluster:currentItem animated:NO];
        }
    }
    
    for (GMSMarker* markerToChange in _markers){
        if ([markerToChange.userData isKindOfClass: POIItem.class]) {
            POIItem* itemInsideMark = (POIItem*) markerToChange.userData;
            if ([itemInsideMark.name isEqualToString:item.name]){
                [markerToChange setIcon: [self getImageFromPath:imageUrl]];
            }
        }
    }
