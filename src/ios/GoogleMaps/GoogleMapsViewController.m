...
- (void)viewDidLoad
{
    ...
    // Enable clustering only if it's set on the js options
    BOOL isClusteringActive = [[initOptions valueForKey:@"clustering"] boolValue];
    if (isClusteringActive) {
        ...
        [[GMUDefaultClusterRenderer alloc] initWithMapView:self.map
                                      clusterIconGenerator:iconGenerator];
        _clusterManager =
        [[GMUClusterManager alloc] initWithMap:self.map
                                     algorithm:algorithm
                                      renderer:renderer];
...
- (BOOL)mapView:(GMSMapView *)mapView didTapMarker:(GMSMarker *)marker {
    ...
    if ([marker.userData isKindOfClass: POIItem.class]) {
        POIItem *poiItem = (POIItem*) marker.userData;
        if (poiItem != nil) {
            NSString *markerPropertyId = [NSString stringWithFormat:@"marker_property_%@", poiItem.name];
...
- (BOOL)clusterManager:(GMUClusterManager *)clusterManager didTapCluster:(id<GMUCluster>)cluster {
    ...
    [GMSCameraPosition cameraWithTarget:cluster.position zoom: self.map.camera.zoom + 1];
...
{
    if ([marker.userData isKindOfClass: POIItem.class]) {
        POIItem* myItem = (POIItem*) marker.userData;
        
        NSString* jsString = [NSString stringWithFormat:@"plugin.google.maps.Map._onMarkerEvent('%@', 'marker_%@');",
                              eventName, myItem.name] ;
